...
- (void) findEyes:(Mat)frame_gray withFace: (cv::Rect) face output:(Mat&) outputFrame {
    ...
    if((int)framesAtPosition > 15) {
        NSLog(@"Here");
        if ((rightPupil.y + leftPupil.y) / 2 > 35){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Your Eyes Moved"
                                                            message:@"They are looking up"
                                                           delegate:self
